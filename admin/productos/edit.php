  ...
  if (!$errores) {
    $sql = "UPDATE productos
            SET nombre=:n, descripcion=:d, precio=:p, stock=:s,
                categoria_id=:c, pais_id=:pa, imagen=:img
            WHERE id=:id";
    $up = $pdo->prepare($sql);
    $up->execute([
      'n'   => $nombre,
...
<div class="container py-4">
  <h4 class="mb-3">Editar producto #<?= (int)$prod['id'] ?></h4>
      ...
      <div class="col-md-8">
        <div class="mb-3">
          ...
          <input type="text" name="nombre" class="form-control" required
                 value="<?= htmlspecialchars($_POST['nombre'] ?? $prod['nombre']) ?>">
        </div>
        <div class="mb-3">
          <label class="form-label">Descripción</label>
          <textarea name="descripcion" class="form-control" rows="4"><?= htmlspecialchars($_POST['descripcion'] ?? $prod['descripcion']) ?></textarea>
        </div>
        <div class="row">
          <div class="col-md-4 mb-3">
            <label class="form-label">Precio</label>
            <input type="number" step="0.01" name="precio" class="form-control" required
                   value="<?= htmlspecialchars($_POST['precio'] ?? $prod['precio']) ?>">
          </div>
          <div class="col-md-4 mb-3">
            <label class="form-label">Stock</label>
            <input type="number" name="stock" class="form-control"
                   value="<?= htmlspecialchars($_POST['stock'] ?? $prod['stock']) ?>">
          </div>
          <div class="col-md-4 mb-3">
            <label class="form-label">Imagen (URL)</label>
            <input type="text" name="imagen" class="form-control"
                   value="<?= htmlspecialchars($_POST['imagen'] ?? $prod['imagen']) ?>">
          </div>
        </div>
      </div>
        ...
        <div class="mb-3">
          ...
          <select name="categoria_id" class="form-select">
            ...
            <?php foreach($cats as $c): ?>
              <option value="<?= (int)$c['id'] ?>" <?= ((($_POST['categoria_id'] ?? $prod['categoria_id'])==$c['id'])?'selected':'') ?>>
        ...
        <div class="mb-3">
          ...
          <select name="pais_id" class="form-select">
            ...
            <?php foreach($paises as $pa): ?>
              <option value="<?= (int)$pa['id'] ?>" <?= ((($_POST['pais_id'] ?? $prod['pais_id'])==$pa['id'])?'selected':'') ?>>
